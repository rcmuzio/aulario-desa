<script>
    ...
    onMount(async () => {
        const res = await apiFetch({
            ruta: `/api/mantenimiento/tipo_articulo?id=${data.id}`,
            method: "GET",
            token: false,
            body: "",
        });
    ...
    async function procesar() {
        if (method != "DELETE") {
            ...
            if (FILA.nombre.trim().length == 0)
                mm.push("El nombre o denominación no es válido.");
        ...
        const res = await apiFetch({
            ruta: `/api/mantenimiento/tipo_articulo`,
            method,
            token: false,
            body: {
                id: FILA.id,
        ...
        if (res.ok) {
            ...
            if (jsn.error) {
                ...
                goto("/app/mantenimiento/tipo_articulo");
            }
        } else {
    ...
    <div class="w3-padding">
        <div class="w3-display-container" style="height: 50px;">
            ...
            <div class="w3-display-right">
                <a href="/app/mantenimiento/tipo_articulo">
